...
class ConsultingAssistant:
    ...
    async def refresh_thread(self):
        ...
        if self.user_thread:
            ...
            logger.info(f"🔄 Thread context refreshed: {self.user_thread.thread_id}")
    ...
    async def chat(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.context and self.context.available_documents:
                ...
                logger.info(f"📂 Prepended document context: {', '.join(self.context.available_documents)}")
            ...
            logger.info(f"🔍 Extracting citations from response object...")
            ...
            if not citations and self.context:
                logger.info(f"   ℹ️ No structured citations. Attempting to extract from response text...")
                logger.info(f"   📂 Available documents in context: {self.context.available_documents}")
